...
> **배운 점**
>
> - **작업 컨텍스트 구조화**: 작업이 길어질수록 AI가 진행 맥락과 완료 조건을 놓치는 문제를 줄이기 위해 할 일을 Task로 나누고, Figma MCP와 개발 문서로 디자인·모듈 구조·상태 관리 규칙을 제공. AI의 작업 진행 능력은 긴 프롬프트보다 필요한 맥락을 적절한 시점에 제공하는 구조에 더 크게 좌우된다는 점을 확인
...
#### 화면 이탈로 취소된 코루틴을 API 실패로 노출한 문제 해결

<span style="font-size: 0.9em;">Reed · Android · Kotlin Coroutines · Circuit · Compose</span>

- **문제**: 도서 항목을 동시에 눌러 상세 화면이 중복으로 쌓인 뒤 빠르게 뒤로 가면, 내 서재 화면에 코루틴 오류가 Toast로 노출됨
- **원인 분석**: 상세 화면이 연속으로 pop되면서 `LaunchedEffect`의 scope와 진행 중인 API 요청이 취소됨. 이때 발생한 `CancellationException`을 `catch (Throwable)`이 일반 통신 오류로 처리해 사용자에게 실패 메시지를 표시한 사실을 확인
- **해결**: `CancellationException`은 다시 throw해 상위 scope로 취소 신호를 전달하고, 나머지 예외만 공통 오류 처리기로 전달. 서로 다른 항목을 동시에 누를 수 없도록 멀티 터치도 차단해 상세 화면의 중복 적재 방지
- **결과**: 화면 생명주기에 따른 정상 취소와 실제 API 실패를 분리하고, 빠른 연속 조작에서도 잘못된 오류 메시지와 중복 화면 전환이 발생하지 않도록 개선

[기술 블로그](https://velog.io/@mraz3068/Coroutine-CancellationException-UseCase)
...
> **배운 점**
>
> - **일회성 이벤트 모델링**: 이벤트 유실을 피하려 State로 관리했지만, 소비 상태를 초기화하지 않아 화면 복원 시 같은 스낵바가 다시 노출되는 문제를 겪음. 이후 자료형보다 이벤트의 유실·소비·복원 시점을 먼저 정의하는 기준을 세움
> - **화면 이탈과 코루틴 취소**: 화면 이탈로 Presenter의 작업이 취소되는 것은 실패가 아니라 생명주기에 따른 정상 종료일 수 있으므로, `CancellationException`을 일반 오류와 구분해 정상 취소와 실제 실패를 나누는 오류 처리 기준 정립
> - **모듈 간 의존성 경계**: Hilt에서 Metro로 전환하던 중 public API가 DataStore 타입을 노출하고 있는데도 해당 의존성을 `implementation`으로 선언해 컴파일러가 타입을 해석하지 못하는 문제를 겪음. 최소 재현 프로젝트로 원인을 좁히며 `api`와 `implementation`의 선택 기준은 관례가 아니라 public API의 타입 노출 여부임을 확인
...
#### 웨이팅 신청 버튼의 상태가 갱신되지 않는 문제 해결

<span style="font-size: 0.9em;">유니페스 · Android · Jetpack Compose · Snapshot State · derivedStateOf</span>

- **문제**: 웨이팅 인원·전화번호·개인정보 동의 조건을 모두 충족해도 신청 버튼이 활성화되지 않아 현장 웨이팅을 등록할 수 없었음
- **원인 분석**: `derivedStateOf` 내부의 `partySize`와 `isPrivacyPolicyClicked`는 Compose State가 아닌 일반 변수였음. `remember` key에도 두 값을 지정하지 않아 Snapshot 시스템이 변경을 추적하지 못하고 초기 값으로 유효성을 계속 계산한 사실을 확인
- **해결**: 일반 변수인 인원 수와 동의 여부를 `remember` key로 명시하고, Snapshot State인 `TextFieldState.text`는 `derivedStateOf` 내부에서 관찰하도록 의존성 구성 수정
- **결과**: 입력 조건에 맞춰 신청 버튼이 갱신되도록 복구하고, 전화번호 입력마다 버튼을 사용하는 UI 전체가 불필요하게 재구성되지 않도록 검증 로직 정리
...
> - iOS `actual` 구현에서 Kotlin/Native로 UIKit·Foundation·CoreGraphics를 직접 다루고 `ImageBitmap`을 `UIImage`로 변환하는 과정에서 픽셀 배치·색 공간·알파 채널·바이트 순서에 따라 색상이 변하는 문제를 겪음. 이를 해결하며 KMP가 플랫폼 코드를 줄여주더라도 플랫폼별 이미지 처리 원리와 네이티브 동작까지 함께 이해해야 한다는 점을 체감
...
> - 기존 수강·알림 흐름을 유지한 채 화면과 상태 관리 책임을 단계적으로 옮기며, 운영 중인 제품의 리팩토링은 새 기술 도입보다 기존 동작을 깨뜨리지 않는 전환 순서가 중요함을 배움
...
### Hi-me - HD현대 그룹사 임직원 대상 건강관리 헬스케어 서비스 <span style="margin-left: 0.75em; font-size: 0.85em; color: #9ca3af; font-weight: normal;">2025.10 ~</span>

**Android 개발** · [Play Store](https://play.google.com/store/apps/details?id=com.mediplussolution.hime) · 다운로드 **2,000+**

Hi-me를 개발·운영하며 해결한 주요 문제를 원인 분석과 해결 결과 중심으로 정리했습니다.

#### WebView Renderer 종료 후 사진 업로드 흐름 복구

<span style="font-size: 0.9em;">Hi-me · Android · WebView · Frontend · Crashlytics</span>
...
<span style="font-size: 0.9em;">Hi-me · Android · Jetpack Compose · AndroidView · WebView · Pointer Input</span>
...
<span style="font-size: 0.9em;">Hi-me · Android · WebView Bridge · Media3 · MediaSession</span>
...
<span style="font-size: 0.9em;">Hi-me · Android · BLE · GATT · DataStore · Bluetooth</span>
